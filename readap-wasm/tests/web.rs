...
fn test_api_instantiation() {
    ...
    assert_eq!(constraint_str, "", "Empty constraint builder should return empty string");
    ...
    assert!(result.is_object(), "Parser should return an object for detailed parsing");
}
...
fn test_immutable_dataset_chaining() {
    ...
    assert!(dataset_with_das.is_ok(), "Adding DAS data should not cause aliasing errors");
    ...
    if let Ok(dataset_with_das) = dataset_with_das {
        ...
        assert!(dataset_with_both.is_ok(), "Adding DDS data should not cause aliasing errors");
        ...
        if let Ok(final_dataset) = dataset_with_both {
            ...
            assert!(var_names.length() >= 0, "Should be able to get variable names");
        }
    }
}
...
fn test_constraint_builder_chaining() {
    ...
    assert!(constraint_str.contains("time"), "Constraint should contain time selection");
    assert!(constraint_str.contains("lat"), "Constraint should contain lat selection");
    assert!(constraint_str.contains("lon"), "Constraint should contain lon selection");
    ...
    assert!(constraint_str2.contains("depth"), "Should be able to continue chaining after build");
}
...
fn test_dods_parser_smoke() {
    ...
    assert!(empty_result.is_object(), "Parser should return object for empty data");
    ...
    assert!(result.is_object(), "Parser should return object for mock data");
    
    // Test structure analysis
    let analysis = parser.analyze_dods_structure(&data_array);
    assert!(analysis.is_object(), "Structure analysis should return object");
}